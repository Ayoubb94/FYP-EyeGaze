...
def read_files(filename, DATADIR):
    # path
    ...
    # raise exception if the file does not exist
        ...
        raise Exception(
            "No eye data file file found for participant '%s'" % (filename))
    ...
    x = []
    y = []
    events = {'Sfix': [], 'Ssac': [], 'Efix': [], 'Esac': [], 'msg': []}
    ...
    # events
    trial['events']['Sfix'], trial['events']['Efix'] = detect_fixations(
        trial['x'], trial['y'], trial['time'])
    trial['events']['Ssac'], trial['events']['Esac'] = detect_saccades(
        trial['x'], trial['y'], trial['time'])
...
def detect_fixations(x, y, time, missing=0.0, maxdist=25, mindur=50):
    ...
    for i in range(1, len(x)):
        ...
        if dist <= maxdist and not fixstart:
            ...
            if time[i-1]-Sfix[-1][0] >= mindur:
                Efix.append([Sfix[-1][0], time[i-1], time[i-1] -
                             Sfix[-1][0], x[si], y[si]])
...
def detect_saccades(x, y, time, missing=0.0, minlen=5, maxvel=40, maxacc=340):
    ...
    Esac - ist of lists, each containing [starttime, endtime, duration, startx, starty, endx, endy]
    """
